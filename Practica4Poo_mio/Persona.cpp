         ...
         ,fNacimiento(),fAlta(0),fBaja(0)
{
    ...
    cout << "Se está creando la persona "<<id<<" "+nombre<<endl;
}
...
{
    ID++;//Una copia
    cout << "Se está copiando a "+orig.nombre+" con nuevo id "<< id <<endl;
...
Persona::~Persona() {
    cout << "Se está destruyendo a "+nombre+" con id "<<id<<endl;
...
string Persona::toCSV() const {
    ...
    ss<<nombre<<";"<<apellidos<<";"<<dni<<";"<<fNacimiento.GetDia()<<";"
            <<fNacimiento.GetMes()<<";"<<fNacimiento.GetAnio();
